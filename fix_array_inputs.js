...
const updateListCode = "const updateList = (listName, id, field, val) => { if (listName === 'expList') setExpList(prev => prev.map(x => x.id === id ? { ...x, [field]: val } : x)); if (listName === 'eduList') setEduList(prev => prev.map(x => x.id === id ? { ...x, [field]: val } : x)); if (listName === 'langList') setLangList(prev => prev.map(x => x.id === id ? { ...x, [field]: val } : x)); };";

let marker = "const startApply = (id: number) => {";
if (code.indexOf("updateList =") === -1) {
  let insertPos = code.indexOf(marker);
  if (insertPos !== -1) {
    code = code.substring(0, insertPos) + updateListCode + '\n  ' + code.substring(insertPos);
  }
}
...
console.log('Fixed array inputs');
